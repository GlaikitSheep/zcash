#!/usr/bin/env python3
#
# Execute all of the automated tests related to VoteCoin.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/votecoin/checksec.sh'), '--file=' + repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/votecoin/checksec.sh'), '--fortify-file=' + repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if votecoind is an ELF binary, they all are
    with open(repofile('src/votecoind'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/votecoind')
    ret &= test_rpath_runpath('src/votecoin-cli')
    ret &= test_rpath_runpath('src/votecoin-gtest')
    ret &= test_rpath_runpath('src/votecoin-tx')
    ...
    # ret &= test_fortify_source('src/votecoind')
    # ret &= test_fortify_source('src/votecoin-cli')
    # ret &= test_fortify_source('src/votecoin-gtest')
    # ret &= test_fortify_source('src/votecoin-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/votecoin-gtest')],
