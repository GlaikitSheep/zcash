...
usage() {
    ...
    echo " [-l,--logfile <logfile>] # default is ./votecoin-build-wrapper.log"
    echo " [-h,--help]"
    echo ""
...
then
    export LOGFILE=./votecoin-build-wrapper.log
fi
...
# sneak the variable into votecoind's build.sh
...
CONFIGURE_FLAGS="--enable-tests=no --disable-bench" \
    "$ZCUTIL/build.sh" \
    -j$(nproc) \
    "CC=$ZCUTIL/libfuzzer/votecoin-wrapper-clang" \
    "CXX=$ZCUTIL/libfuzzer/votecoin-wrapper-clang++" "${POSITIONAL[@]:1}" || die "Build failed at stage $BUILD_STAGE."
