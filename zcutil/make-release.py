...
def main(args=sys.argv[1:]):
    """
    Perform the final VoteCoin release process up to the git tag.
    """
...
def verify_dependency_updates():
    try:
        sh_log('./qa/votecoin/updatecheck.py')
    except SystemExit:
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libvotecoin_script.pc',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "votecoin-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "votecoin-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './votecoin-make-release.log'
    ...
    logging.info('votecoin make-release.py debug log: %r', logname)
