...
function help {
    cat <<EOF
Start fuzzing a case in a previously votecoind built for AFL.
This script must be run from within the top level directory of a votecoin clone.
...
"$AFL_INSTALL_DIR/afl-fuzz" -i "./src/fuzzing/$FUZZ_CASE/input" -o "./src/fuzzing/$FUZZ_CASE/output" "$@" ./src/votecoind @@
