...
Nathan Wilcox (4):
      ...
      Add a votecoin-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the votecoin-gtest run; invoke with make votecoin-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to votecoin-gtest.
      ...
      Small nit fixes
      Run `votecoin-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libvotecoin` VoteCoin protocol API and crypto constructions surrounding the zkSNARK circuit.
      ...
      Rename VOTECOIN_ constants to ZC_.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to votecoin.conf and votecoind.pid in qa/ and src/
