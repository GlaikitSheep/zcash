...
[Pull request](https://github.com/votecoin/votecoin/pull/3559)
...
[Pull request](https://github.com/votecoin/votecoin/pull/3492), [ZIP 32](https://github.com/votecoin/zips/blob/master/zip-0032.mediawiki)

Fix Signing Raw Transactions with Unsynced Offline Nodes
--------------------------------------------------------
With v2.0.0, in `signrawtransaction` the consensus branch ID (which is used to construct the transaction) was estimated using the chain height. With v2.0.1 this has been improved by also considering the `APPROX_RELEASE_HEIGHT` of the release, and a new parameter to allow the caller to manually override the consensus branch ID that votecoind will use.

[Pull request](https://github.com/votecoin/votecoin/pull/3520)
...
Jack Grigg (83):
      ...
      Clean up libvotecoin CPPFLAGS, CXXFLAGS, and LDFLAGS
      ...
      Metrics UI: Tell Windows users how to stop votecoind
...
Jonas Schnelli (3):
      [Wallet] extend CKeyMetadata with HD keypath     VoteCoin: modified for zip32
      [Wallet] print hd masterkeyid in getwalletinfo     VoteCoin: modified for zip32
      [Wallet] ensure CKeyMetadata.hdMasterKeyID will be cleared during SetNull()     VoteCoin: modified for zip32
