...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    if (canopyEnabled) {
        check_funding_streams(obj, {"Electric Coin Company", "VoteCoin Foundation", "Major Grants" },
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_exportwallet)
{
    ...
    libvotecoin::SproutPaymentAddress addr = pwalletMain->GenerateNewSproutZKey();
    pwalletMain->GetSproutPaymentAddresses(addrs);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    auto testSpendingKey = libvotecoin::SproutSpendingKey::random();
    ...
    std::string format_str = "# Wallet dump created by VoteCoin v0.11.2.0.z8-9155cc6-dirty (2016-08-11 11:37:00 -0700)\n"
    ...
    BOOST_ASSERT(std::get_if<libvotecoin::SproutPaymentAddress>(&address) != nullptr);
    auto addr = std::get<libvotecoin::SproutPaymentAddress>(address);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importexport)
{
    ...
    pwalletMain->GetSproutPaymentAddresses(addrs);
    BOOST_CHECK(addrs.size()==0);
    std::set<libvotecoin::SaplingPaymentAddress> saplingAddrs;
...
void CheckHaveAddr(const libvotecoin::PaymentAddress& addr) {
...
BOOST_AUTO_TEST_CASE(rpc_wallet_encrypted_wallet_sapzkeys)
{
    ...
    // wallet should currently be empty
    std::set<libvotecoin::SaplingPaymentAddress> addrs;
