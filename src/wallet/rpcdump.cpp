...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"votecoinprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"votecoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid VoteCoin address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid VoteCoin spending key
        if (fImportZKeys) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                // Not a valid spending key, so carry on and see if it's a VoteCoin style t-address.
            }
        }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by votecoind -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the votecoind -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by VoteCoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (fDumpZKeys) {
        std::set<libvotecoin::SproutPaymentAddress> sproutAddresses;
        ...
        std::set<libvotecoin::SaplingPaymentAddress> saplingAddresses;
...
UniValue z_importkey(const UniValue& params, bool fHelp)
{
    ...
    auto addrInfo = std::visit(libvotecoin::AddressInfoFromSpendingKey{}, spendingkey);
...
UniValue z_importviewingkey(const UniValue& params, bool fHelp)
{
    ...
    auto addrInfo = std::visit(libvotecoin::AddressInfoFromViewingKey{}, viewingkey);
    UniValue result(UniValue::VOBJ);
