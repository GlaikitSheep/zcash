...
const unsigned char VOTECOIN_HD_SEED_FP_PERSONAL[BLAKE2bPersonalBytes] =
    {'Z', 'c', 'a', 's', 'h', '_', 'H', 'D', '_', 'S', 'e', 'e', 'd', '_', 'F', 'P'};

const unsigned char VOTECOIN_TADDR_OVK_PERSONAL[BLAKE2bPersonalBytes] =
...
{
    CBLAKE2bWriter h(SER_GETHASH, 0, VOTECOIN_HD_SEED_FP_PERSONAL);
    h << seed;
    return h.GetHash();
}
...
uint256 ovkForShieldingFromTaddr(HDSeed& seed) {
    ...
    auto state = blake2b_init(64, VOTECOIN_TADDR_OVK_PERSONAL);
...
std::optional<std::pair<diversifier_index_t, libvotecoin::SaplingPaymentAddress>>
    SaplingExtendedFullViewingKey::Address(diversifier_index_t j) const
{
    ...
    CSerializeData addr_bytes(libvotecoin::SerializedSaplingPaymentAddressSize);
    if (librustzcash_zip32_xfvk_address(
        ...
        libvotecoin::SaplingPaymentAddress addr;
        ss_addr >> addr;
...
libvotecoin::SaplingPaymentAddress SaplingExtendedFullViewingKey::DefaultAddress() const
{
...
libvotecoin::SaplingPaymentAddress SaplingExtendedSpendingKey::DefaultAddress() const
{
