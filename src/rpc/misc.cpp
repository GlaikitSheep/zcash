...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total VoteCoin balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"votecoinaddress\"\n"
            "\nReturn information about the given VoteCoin address.\n"
            "\nArguments:\n"
            "1. \"votecoinaddress\"     (string, required) The VoteCoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"votecoinaddress\",   (string) The VoteCoin address validated\n"
...
{
public:
    UniValue operator()(const libvotecoin::InvalidEncoding &zaddr) const { return UniValue(UniValue::VOBJ); }

    UniValue operator()(const libvotecoin::SproutPaymentAddress &zaddr) const {
        UniValue obj(UniValue::VOBJ);
    ...
    UniValue operator()(const libvotecoin::SaplingPaymentAddress &zaddr) const {
        UniValue obj(UniValue::VOBJ);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are VoteCoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) VoteCoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"votecoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"votecoinaddress\"    (string, required) The VoteCoin address to use for the signature.\n"
...
UniValue getaddressmempool(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressmempool is disabled. "
            "Run './votecoin-cli help getaddressmempool' for instructions on how to enable this feature.");
    }
...
UniValue getaddressutxos(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressutxos is disabled. "
            "Run './votecoin-cli help getaddressutxos' for instructions on how to enable this feature.");
    }
...
UniValue getaddressdeltas(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressdeltas is disabled. "
            "Run './votecoin-cli help getaddressdeltas' for instructions on how to enable this feature.");
    }
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressbalance is disabled. "
            "Run './votecoin-cli help getaddressbalance' for instructions on how to enable this feature.");
    }
...
UniValue getaddresstxids(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddresstxids is disabled. "
            "Run './votecoin-cli help getaddresstxids' for instructions on how to enable this feature.");
    }
...
UniValue getspentinfo(const UniValue& params, bool fHelp)
{
    ...
    if (!fExperimentalInsightExplorer) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getspentinfo is disabled. "
            "Run './votecoin-cli help getspentinfo' for instructions on how to enable this feature.");
    }
