...
inline int set_error(votecoin_script_error* ret, votecoin_script_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
    ...
    votecoin_script_error* err)
{
    try {
        ...
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen) {
            set_error(err, votecoin_script_ERR_TX_SIZE_MISMATCH);
            return nullptr;
        }

        // Regardless of the verification result, the tx did not error.
        set_error(err, votecoin_script_ERR_OK);
        auto preTx = new PrecomputedTransaction(tx);
        return preTx;
    } catch (const std::exception&) {
        set_error(err, votecoin_script_ERR_TX_DESERIALIZE); // Error deserializing
        return nullptr;
    }
}

void votecoin_script_free_precomputed_tx(void* pre_preTx)
{
...
int votecoin_script_verify_precomputed(
    ...
    votecoin_script_error* err)
{
    ...
    if (nIn >= preTx->tx.vin.size())
        return set_error(err, votecoin_script_ERR_TX_INDEX);
    ...
    votecoin_script_error* err)
{
    try {
        ...
        if (nIn >= tx.vin.size())
            return set_error(err, votecoin_script_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, votecoin_script_ERR_TX_SIZE_MISMATCH);
        ...
        return set_error(err, votecoin_script_ERR_TX_DESERIALIZE); // Error deserializing
    }
}

unsigned int votecoin_script_version()
{
    // Just use the API version for now
    return VOTECOIN_SCRIPT_API_VER;
}
